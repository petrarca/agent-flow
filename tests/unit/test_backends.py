...
Three layers, tested where each belongs:
  ...
  3. LocalBackend's own primitives (threadpool _execute_parallel, semaphore,
...
(test_flow_backends.py), parametrized alongside local. This keeps the unit suite
Prefect-free (see test_prefect_isolation.py).
"""

import pytest

from agent_flow.backends import DEFAULT_BACKEND, FlowBackend, LocalBackend, get_backend
...
def test_get_backend_local():
    b = get_backend("local")
    assert isinstance(b, LocalBackend) and isinstance(b, FlowBackend) and b.name == "local"


def test_get_backend_fresh_instances():
    # Fresh instance per call keeps per-run state (LocalBackend's semaphore) isolated.
...
# --- LocalBackend-specific primitives ---------------------------------------


def test_local_parallel_threadpool_runs_all():
    ran = []
    out = LocalBackend()._execute_parallel(["p1", "p2", "p3"], _run_node_factory(ran))
    assert set(out) == {"p1", "p2", "p3"} and all(o.status == "ok" for o in out.values())


def test_local_parallel_degrades_on_exception():
    out = LocalBackend()._execute_parallel(["ok", "bad"], _run_node_factory([], fail=("bad",)))
    assert out["ok"].status == "ok" and out["bad"].status == "degraded"


def test_local_parallel_propagates_node_blocked():
    with pytest.raises(NodeBlocked):
        LocalBackend()._execute_parallel(["ok", "block"], _run_node_factory([], block=("block",)))
