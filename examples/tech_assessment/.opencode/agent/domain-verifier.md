...
# Verifier: domain-verifier (simulated)
...
sections are missing), include `"rerun_required": ["domain"]` in your control
JSON instead of a bare `"verified"` status, so the domain step is redone. This
