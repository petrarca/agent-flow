...
# Verifier: tech-stack-verifier (simulated)
...
sections are missing), include `"rerun_required": ["tech-stack"]` in your
control JSON instead of a bare `"verified"` status, so the tech-stack step is
redone. This should be rare — only for a genuinely unusable report.
